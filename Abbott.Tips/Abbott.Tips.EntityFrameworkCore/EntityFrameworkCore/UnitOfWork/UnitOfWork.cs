using Abbott.Tips.EntityFrameworkCore.Repository;
...
namespace Abbott.Tips.EntityFrameworkCore.UnitOfWork
{
    /// <summary>
    ...
    public class UnitOfWork<TContext> : IEntityRepositoryFactory, IGenericUnitOfWork<TContext>, IUnitOfWork where TContext : DbContext
    {
        ...
        /// <returns>An instance of type inherited from <see cref="IEntityRepository{TEntity}"/> interface.</returns>
        ...
        #region 自定义执行SQL语句或存储过程

        public IList<TEntity> SqlQuery<TEntity>(string procedureCommand, params SqlParameter[] sqlParams) where TEntity : class
        {
            return _context.Query<TEntity>().FromSql(procedureCommand, sqlParams).ToList();
        }
        ...
        /// <returns></returns>
        public int ExecuteProcedure(string procedureCommand, params SqlParameter[] sqlParams)
        {
            return _context.Database.ExecuteSqlCommand(procedureCommand, sqlParams);
        }

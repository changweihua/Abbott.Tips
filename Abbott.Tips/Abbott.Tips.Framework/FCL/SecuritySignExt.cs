...
{
    public static class SecuritySignExt
    {
        ...
        public static string GetSecuritySign(this NameValueCollection getCollection, string partner, string partnerKey, NameValueCollection postCollection = null)
        {
            ...
            var dic = SecuritySignExt.GetSortedDictionary(getCollection,
                (k) =>
                {//过滤partner及sign
                    return string.Equals(k, SecuritySignExt.Partner, StringComparison.OrdinalIgnoreCase)
                        || string.Equals(k, SecuritySignExt.Sign, StringComparison.OrdinalIgnoreCase);
                });
            dic.Add(SecuritySignExt.Partner, partner);
            StringBuilder tmp = new StringBuilder();
            SecuritySignExt.FillStringBuilder(tmp, dic);//将QueryString填入StringBuilder
            dic = SecuritySignExt.GetSortedDictionary(postCollection);
            SecuritySignExt.FillStringBuilder(tmp, dic);//将Form填入StringBuilder
